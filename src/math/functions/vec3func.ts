...
//no need to actually import Vec3, just need type info
 ...
 */
export function copy(out: Vec3Like, a: Vec3Like): Vec3Like {
  out[0] = a[0];
  out[1] = a[1];
  out[2] = a[2];
  return out;
...
const angle_tempA = [0,0,0];
const angle_tempB = [0,0,0];
...
export function angle(a: Vec3Like, b: Vec3Like): number {
  ...
  normalize(angle_tempA, angle_tempA);
  normalize(angle_tempB, angle_tempB);

  let cosine = dot(angle_tempA, angle_tempB);
 ...
 */
export function exactEquals(a: Vec3Like, b: Vec3Like): boolean {
  return a[0] === b[0] && a[1] === b[1] && a[2] === b[2];
}